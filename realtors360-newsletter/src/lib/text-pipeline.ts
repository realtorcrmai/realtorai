 ...
 *   7. Basic quality checks — non-empty, reasonable length, personalized
 */
...
export const LENGTH_TARGETS: Record<string, { min: number; max: number; label: string }> = {
  ...
  birthday: { min: 15, max: 60, label: "Birthday emails should be short and warm: 15-60 words" },
  price_drop: { min: 40, max: 100, label: "Price drop alerts should be 40-100 words" },
  showing_confirmed: { min: 30, max: 80, label: "Showing confirmations should be concise: 30-80 words" },
};
...
export async function runTextPipeline(db: SupabaseClient, input: PipelineInput): Promise<PipelineResult> {
  ...
  try {
    ...
    const recentSubjects = (recentEmails || []).map((e: { subject: string }) => e.subject.toLowerCase());
    ...
    const similar = recentSubjects.filter((s: string) => s.slice(0, 20) === subjectPrefix);
    ...
    logger.warn({ err }, 'Subject dedup DB check failed — skipping');
  }
