<!-- docs-audit: src/actions/newsletters.ts, src/emails/*, realtors360-newsletter/** -->
# PRD: AI Newsletter & Journey Engine — Realtors360 CRM

**Version:** 2.0
**Date:** March 26, 2026
**Author:** Realtors360 Team
**Status:** Implemented (with additions beyond original spec)

---

## 0. Implementation Status (March 26, 2026)

> This section reflects what is actually built vs the original spec below.

### What Changed From Original PRD

**UI Tabs: 7 → 6**
The original spec described 7 tabs (Overview, AI Agent, Campaigns, Relationships, Journeys, Analytics, Settings). The implementation consolidated these to 6 tabs:

| Current Tab | What It Contains | Original Tabs Merged |
|-------------|-----------------|---------------------|
| Overview | Health pills, Act Now urgency card, pipeline, AI activity | Overview + Relationships (pipeline, health) |
| AI Workflows | 7 workflow cards with step counts, links to detail pages | New (not in original PRD) |
| Performance | AI Working For You stats, approval queue, sent emails, held back | AI Agent + Analytics |
| Campaigns | Manual listing blast wizard, custom campaign wizard, blast history | Campaigns (simplified) |
| Automation | Listing Blast Automations (multi-rule), Greeting Automations (11 occasions) | New (not in original PRD) |
| Settings | Master switch, frequency cap, quiet hours, compliance — persisted to DB | Settings (now persists) |

**Removed:** Relationships tab (merged into Overview), Journeys tab (merged into Overview pipeline), Analytics tab (merged into Performance).

### Features Added Beyond Original PRD

1. **AI Agent System** — 10 modules: lead scoring, contact evaluation, greeting agent, next-best-action, send advisor, send governor, timing optimizer, trust manager, trust gate, voice learner
2. **Greeting Automations** — 11 occasions (birthday, home anniversary, Christmas, New Year, Diwali, Lunar New Year, Canada Day, Thanksgiving, Valentine's, Mother's Day, Father's Day). AI agent detects dates, Claude writes unique personalized greeting per contact.
3. **Listing Blast Automations** — Multi-rule system: realtor creates multiple automation rules, each with its own trigger (listing active/created/price change), template, recipients, and approval mode.
4. **BCC Monitoring** — Every outgoing email BCCs the realtor with a metadata banner showing workflow, step, contact, phase, timestamp.
5. **7 Seeded Workflows** — 108 steps total: Buyer Nurture (24), Post-Close Buyer (19), Post-Close Seller (19), Speed-to-Contact (12, **inactive by default** — realtor enables manually and enrolls contacts via UI), Referral Partner (12), Re-Engagement (11), Open House Follow-Up (11).
6. **8 Cron Jobs** — agent-evaluate (10min), agent-scoring (15min), agent-recommendations (hourly), process-workflows (daily), daily-digest (daily), consent-expiry (weekly), weekly-learning (weekly).
7. **Apple-Quality Email Blocks** — 14 modular blocks with SF Pro font, dark mode, responsive design. 11 template definitions.
8. **Validation Pipeline** — 7-step: content → design → compliance → quality scoring → trust gate → send. Greeting emails skip quality scoring.
9. **Test Suite** — 1860 automated UI tests, 46 email delivery tests, 1170 documented test cases across 16 categories.

### Contact Types (6 types)

| Type | Purpose | Workflows | Emails |
|------|---------|-----------|--------|
| **customer** | Unqualified lead — hasn't declared buy/sell intent | Welcome + market updates only | Generic nurture until qualified |
| **buyer** | Qualified — actively looking to buy | Buyer Nurture, Open House Follow-Up, Post-Close Buyer | Listing alerts, mortgage guide, offers, closing |
| **seller** | Has or wants to list property | Post-Close Seller | CMA preview, showing summaries, sale congrats |
| **agent** | Cooperating agents from other brokerages | None — not clients | Listing blasts ONLY (no greetings, no nurture) |
| **partner** | Referral sources (brokers, lawyers, inspectors) | Referral Partner | Partner welcome, market updates, quarterly newsletter |
| **other** | Catch-all | None | None by default |

**Key behavior:** Lead capture webhook defaults to `customer` type. Realtor qualifies and converts to `buyer` or `seller`. Agents only receive listing blasts. Partners receive referral nurture. Greetings exclude agents.

### Features Not Yet Built (From Original PRD)

| Feature | PRD Section | Status |
|---------|-------------|--------|
| Custom campaign wizard actual send | 4.4 | UI exists, send not wired for custom campaigns |
| Listing blast auto-trigger on status change | 4.4 | Rules save to DB but no event listener fires them |
| A/B testing | 4.5 | Migration exists (014), no UI |
| Revenue attribution | 4.4 | Data exists, no UI dashboard |
| SMS/WhatsApp via Twilio in workflows | 4.2 | Code exists, not tested in production |

### Key Documentation

| Document | Location |
|----------|----------|
| Production Deployment | `realestate-crm/docs/PRODUCTION_DEPLOYMENT.md` |
| Tech Debt (19 items) | `realestate-crm/docs/TECH_DEBT.md` |
| Test Plan (1170 cases) | `realestate-crm/docs/TEST_PLAN_1000.md` |
| Architecture in CLAUDE.md | `realestate-crm/CLAUDE.md` → "AI Email Marketing Engine" section |
