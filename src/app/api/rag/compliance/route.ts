...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await db.rpc('rag_export_user_data', {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { data, error } = await db.rpc('rag_delete_user_data', {
      p_user_email: session.user.email,
    });
    ...
    if (error) {
      // Fallback if RPC doesn't exist yet
      if (error.code === '42883') {
        await db.from('rag_sessions').delete().eq('user_email', session.user.email);
        await db.from('rag_audit_log').delete().eq('user_email', session.user.email);
