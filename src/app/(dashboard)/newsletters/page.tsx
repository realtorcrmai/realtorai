...
export default async function NewsletterDashboard() {
  ...
  const [
    ...
    supabase
      .from("newsletters")
      .select("id, subject, email_type, status, sent_at, created_at, contact_id, contacts(name, type)")
      .order("created_at", { ascending: false })
      .limit(20),
    supabase
      .from("contact_journeys")
      .select("id, contact_id, journey_type, current_phase, is_paused, next_email_at, send_mode, contacts(name, type, email)")
      .order("created_at", { ascending: false }),
    supabase
      .from("workflows")
      .select("id, name, slug, is_active, trigger_type")
      .order("name"),
    supabase
      .from("listings")
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          AI-powered email marketing — one page, full control
        </p>
      </div>
        ...
        {{
                  ...
                  <CardContent className="p-5">
                    ...
                    {phases.map((p) => (
                      <div key={p} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                        <span className="text-sm text-foreground">{phaseIcons[p]} {phaseLabels[p]}</span>
          ...
          queue: (
            <div className="space-y-3">
              ...
              {queue.length === 0 ? (
                <Card><CardContent className="p-8 text-center text-muted-foreground text-sm">No pending emails. The AI will generate drafts as contacts progress through their journeys.</CardContent></Card>
              ) : (
                queue.map((n: any) => (
                  <Card key={n.id}>
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-foreground">{n.contacts?.name || "Unknown"}</p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  { emoji: "🎂", name: "Home Anniversary", rate: "68%" },
                  { emoji: "📢", name: "Listing Blast", rate: "—" },
                ].map((t) => (
              ...
              <Card>
                <CardContent className="p-6 text-center text-muted-foreground">
                  <p className="text-sm">Custom templates coming soon — AI generates templates from your brand config.</p>
                </CardContent>
              </Card>
            </div>
          ),
          ...
          blasts: (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <p className="text-xs text-muted-foreground">Send new listing announcements to all agents</p>
              </div>
              {(!listings || listings.length === 0) ? (
                <Card><CardContent className="p-8 text-center text-muted-foreground text-sm">No active listings. Create a listing to send a blast to agents.</CardContent></Card>
              ) : (
                listings.map((l: any) => (
                  <Card key={l.id}>
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {l.list_price ? `$${Number(l.list_price).toLocaleString()}` : "Price TBD"} · {l.status}
                          </p>
                        </div>
                        <button className="flex items-center gap-1.5 px-3 py-1.5 bg-primary text-white rounded-lg text-xs font-medium hover:bg-primary/90 transition-colors">
                          📧 Send Blast
                        </button>
                      </div>
                    </CardContent>
          ...
          schedule: (
            <div className="space-y-3">
              <h3 className="text-lg font-semibold">Upcoming Emails</h3>
              {(() => {
                const scheduled = (journeys || [])
                  .filter((j: any) => j.next_email_at && !j.is_paused)
                  .sort((a: any, b: any) => new Date(a.next_email_at).getTime() - new Date(b.next_email_at).getTime());
                if (scheduled.length === 0) return <Card><CardContent className="p-8 text-center text-muted-foreground text-sm">No emails scheduled.</CardContent></Card>;
                  ...
                  <Card key={j.id}>
                    <CardContent className="p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium">{j.contacts?.name || "Unknown"}</p>
                          <p className="text-xs text-muted-foreground">{j.journey_type} journey · {j.current_phase.replace(/_/g, " ")} phase</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-medium text-primary">
                            {new Date(j.next_email_at).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}
                          </p>
                          <p className="text-[10px] text-muted-foreground">
                            {new Date(j.next_email_at).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}
                          </p>
                        </div>
                      </div>
                    </CardContent>
          ...
          activity: (
            <div className="space-y-3">
              ...
              {(!recentNewsletters || recentNewsletters.length === 0) ? (
                <Card><CardContent className="p-8 text-center text-muted-foreground text-sm">No email activity yet.</CardContent></Card>
              ) : (
                    ...
                    <CardContent className="p-3">
                      <div className="flex items-center justify-between">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {nl.contacts?.name} · {nl.email_type?.replace(/_/g, " ")} · {nl.sent_at ? new Date(nl.sent_at).toLocaleDateString() : "draft"}
                          </p>
                        </div>
                        <Badge className={`text-[10px] ${
                          nl.status === "sent" ? "bg-emerald-100 text-emerald-700" :
                          nl.status === "draft" ? "bg-amber-100 text-amber-700" :
          ...
          settings: (
            <div className="space-y-4">
                ...
                <CardContent className="p-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-border">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">No emails between these hours</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-border">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Review first or auto-send for new contacts</p>
                    </div>
              ...
              <Card>
                <CardContent className="p-5">
                  <h4 className="text-sm font-semibold mb-3">Workflows ({workflows?.length || 0})</h4>
                  {workflows?.map((w: any) => (
