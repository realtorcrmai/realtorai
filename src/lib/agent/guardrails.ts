...
const GUARDRAIL_PATTERNS: Array<{ pattern: RegExp; type: string }> = [
  ...
  { pattern: /(?:reveal|show|print|output)\s+(?:your\s+)?system\s*prompt/i, type: 'injection' },
  ...
  { pattern: /(?:bypass|override)\s+(?:your\s+)?(?:safety|guardrail|filter|restriction)/i, type: 'injection' },
];
