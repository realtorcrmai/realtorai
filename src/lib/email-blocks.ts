...
// ═══════════════════════════════════════════════
// SPECTACULAR WELCOME EMAIL — Dark luxury template
// Zero AI dependency. 100% template-driven. Pixel-perfect every time.
// ��═══════════════���══════════════════════════════
...
export interface WelcomeEmailData {
  ...
  email: string;
  headshotUrl?: string;
  brandColor?: string; // defaults to gold #c9a96e
  tagline?: string;
  serviceAreas?: string[];
...
export function buildWelcomeEmail(data: WelcomeEmailData): string {
  const e = (s: string | null | undefined) => esc(s);
  const gold = data.brandColor || "#c9a96e";
  const agentName = e(data.agentName);
  const firstName = e(data.contactFirstName);
  const brokerage = e(data.brokerage);
  const title = e(data.agentTitle || "REALTOR®");
  const phone = e(data.phone);
  const areas = data.serviceAreas?.length ? data.serviceAreas.map(a => e(a)).join(" · ") : "your area";
  const ctaUrl = data.ctaUrl || `mailto:${data.email}?subject=Hi ${data.agentName} — I'd like to connect`;
  const initials = data.agentName.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2);

  const headshotHtml = data.headshotUrl
    ? `<img src="${data.headshotUrl}" width="120" height="120" alt="${agentName}" style="display:block;width:120px;height:120px;border-radius:50%;object-fit:cover;margin:0 auto;border:3px solid ${gold};">`
    : `<div style="width:120px;height:120px;border-radius:50%;background:linear-gradient(135deg,${gold},#8a7340);text-align:center;line-height:120px;color:#0a0a0a;font-weight:700;font-size:44px;margin:0 auto;border:3px solid ${gold};">${e(initials)}</div>`;

  const smallHeadshotHtml = data.headshotUrl
    ? `<img src="${data.headshotUrl}" width="52" height="52" alt="${agentName}" style="display:block;width:52px;height:52px;border-radius:50%;object-fit:cover;border:2px solid ${gold};">`
    : `<div style="width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,${gold},#8a7340);text-align:center;line-height:52px;color:#0a0a0a;font-weight:700;font-size:20px;border:2px solid ${gold};">${e(initials)}</div>`;

  // Social links row
  const socials: string[] = [];
  if (data.socialLinks?.instagram) socials.push(`<a href="${data.socialLinks.instagram}" style="color:${gold};text-decoration:none;font-size:13px;font-weight:500;">Instagram</a>`);
  if (data.socialLinks?.facebook) socials.push(`<a href="${data.socialLinks.facebook}" style="color:${gold};text-decoration:none;font-size:13px;font-weight:500;">Facebook</a>`);
  if (data.socialLinks?.linkedin) socials.push(`<a href="${data.socialLinks.linkedin}" style="color:${gold};text-decoration:none;font-size:13px;font-weight:500;">LinkedIn</a>`);
  const socialHtml = socials.length ? `
    <tr><td style="padding:0 48px 32px;text-align:center;">
      <div style="font-size:11px;color:#555;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px;">Connect</div>
      ${socials.join(' <span style="color:#333;margin:0 8px;">·</span> ')}
    </td></tr>` : "";
...
<style>
  @media(max-width:600px){
    .welcome-card{border-radius:0!important}
    .hero-pad{padding:48px 24px!important}
...
<div style="display:none;max-height:0;overflow:hidden;font-size:1px;color:#0a0a0a;">Welcome — ${agentName} is ready to help you find your perfect home.</div>
<table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0a;">
<tr><td align="center" style="padding:0;">
<table width="600" cellpadding="0" cellspacing="0" style="background:#0a0a0a;" class="welcome-card">

<!-- ═══ HERO: Agent presence ═══ -->
<tr><td style="padding:64px 48px 40px;text-align:center;background:linear-gradient(180deg,#111 0%,#0a0a0a 100%);" class="hero-pad">
  <!-- Gold accent line -->
  <div style="width:40px;height:2px;background:${gold};margin:0 auto 32px;border-radius:1px;"></div>
  <!-- Headshot -->
  ${headshotHtml}
  <!-- Name -->
  <div style="font-size:28px;font-weight:700;color:#f5f5f7;margin-top:20px;letter-spacing:-0.5px;">${agentName}</div>
  <div style="font-size:13px;color:${gold};text-transform:uppercase;letter-spacing:2px;margin-top:6px;font-weight:500;">${title}</div>
  <div style="font-size:14px;color:#86868b;margin-top:4px;">${brokerage}</div>
  <!-- Gold divider -->
  <div style="width:60px;height:1px;background:linear-gradient(90deg,transparent,${gold},transparent);margin:24px auto;"></div>
  <!-- Tagline -->
  <div style="font-size:17px;color:rgba(245,245,247,0.8);font-style:italic;letter-spacing:-0.2px;line-height:1.5;">${e(data.tagline || `Your trusted partner in ${areas}`)}</div>
</td></tr>

<!-- ═══ GREETING ═══ -->
<tr><td style="padding:40px 48px 0;" class="content-pad">
  <div style="font-size:16px;color:#f5f5f7;line-height:1.7;">
    Hi ${firstName},
  </div>
  <div style="font-size:15px;color:#a1a1a6;line-height:1.7;margin-top:12px;">
    Thank you for connecting with me. I believe finding the right property should feel exciting, not stressful. That's what I'm here for.
  </div>
</td></tr>

<!-- ═══ GOLD DIVIDER ═══ -->
<tr><td style="padding:32px 48px;" class="content-pad">
  <div style="height:1px;background:linear-gradient(90deg,transparent,#333,transparent);"></div>
</td></tr>

<!-- ═══ 3 COMMITMENTS ═���═ -->
<tr><td style="padding:0 48px;" class="content-pad">
  <div style="font-size:11px;color:${gold};text-transform:uppercase;letter-spacing:2.5px;font-weight:600;margin-bottom:24px;">My Commitments To You</div>
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
      <td width="4" style="background:${gold};border-radius:2px;"></td>
      <td style="padding:16px 0 16px 20px;">
        <div style="font-size:16px;font-weight:600;color:#f5f5f7;letter-spacing:-0.2px;">I will never waste your time</div>
        <div style="font-size:13px;color:#86868b;margin-top:4px;line-height:1.5;">Only relevant properties. No spam. Every recommendation is handpicked for you.</div>
      </td>
    </tr>
    ...
    <tr>
      <td width="4" style="background:${gold};border-radius:2px;"></td>
      <td style="padding:16px 0 16px 20px;">
        <div style="font-size:16px;font-weight:600;color:#f5f5f7;letter-spacing:-0.2px;">I will find what others miss</div>
        <div style="font-size:13px;color:#86868b;margin-top:4px;line-height:1.5;">Early access to new listings, off-market opportunities, and insights the portals don't show.</div>
      </td>
    </tr>
    <tr><td colspan="2" style="height:12px;"></td></tr>
    <tr>
      <td width="4" style="background:${gold};border-radius:2px;"></td>
      <td style="padding:16px 0 16px 20px;">
        <div style="font-size:16px;font-weight:600;color:#f5f5f7;letter-spacing:-0.2px;">I will fight for every dollar</div>
        <div style="font-size:13px;color:#86868b;margin-top:4px;line-height:1.5;">Expert negotiation, market data, and strategy to ensure you get the best possible outcome.</div>
      </td>
    </tr>
  </table>
</td></tr>
...
<!-- ═══ CTA ═══ -->
<tr><td style="padding:0 48px 8px;text-align:center;" class="content-pad">
  <div style="font-size:15px;color:#a1a1a6;margin-bottom:20px;">Ready to get started?</div>
  <a href="${ctaUrl}" style="display:inline-block;background:${gold};color:#0a0a0a;font-size:15px;font-weight:700;text-decoration:none;padding:16px 40px;border-radius:8px;letter-spacing:0.3px;">
    Book a Free Consultation
  </a>
  <div style="margin-top:16px;">
    <a href="tel:${data.phone}" style="color:${gold};text-decoration:none;font-size:14px;font-weight:500;">
      or call ${phone}
    </a>
  </div>
</td></tr>
...
<!-- ═══ SIGNATURE ═���═ -->
<tr><td style="padding:24px 48px 0;" class="content-pad">
  <div style="border-top:1px solid #1c1c1e;padding-top:24px;">
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td width="60">${smallHeadshotHtml}</td>
        <td style="padding-left:16px;">
          <div style="font-size:15px;font-weight:600;color:#f5f5f7;">${agentName}</div>
          <div style="font-size:13px;color:#86868b;">${brokerage}</div>
          <div style="font-size:13px;margin-top:2px;"><a href="tel:${data.phone}" style="color:${gold};text-decoration:none;font-weight:500;">${phone}</a></div>
        </td>
      </tr>
    </table>
  </div>
...
<tr><td style="padding:32px 48px 40px;text-align:center;">
  <p style="font-size:11px;color:#555;margin:0;line-height:1.6;">
    ...
    <a href="${data.unsubscribeUrl ?? '#'}" style="color:#555;text-decoration:underline;">Unsubscribe</a> · <a href="#" style="color:#555;text-decoration:underline;">Privacy</a>
  </p>
  <p style="font-size:11px;color:#444;margin:6px 0 0;">
    ${e(data.physicalAddress || 'Please contact us for our mailing address')}
  </p>
</td></tr>
