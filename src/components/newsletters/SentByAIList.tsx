    ...
    <Card>
      <CardContent className="p-5">
        ...
        <div className="grid grid-cols-4 gap-3 mb-4 p-3 bg-muted/30 rounded-lg">
          ...
          <div className="text-center">
            <div className="text-lg font-bold text-emerald-600">{grouped.reduce((a, g) => a + g.totalOpens, 0)}</div>
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 ${
                      group.contactType === "seller" ? "bg-gradient-to-br from-primary to-orange-500" : "bg-gradient-to-br from-primary to-purple-500"
                    }`}>
                      {group.contactName[0]}
                    </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0 ml-3">
                    ...
                    <div className="flex items-center gap-2.5">
                      <div className="flex items-center gap-1" title="Opens">
                        <MailOpen className={`h-3.5 w-3.5 ${group.totalOpens > 0 ? "text-emerald-500" : "text-muted-foreground/40"}`} />
                        <span className={`text-xs font-medium ${group.totalOpens > 0 ? "text-emerald-600" : "text-muted-foreground"}`}>{group.totalOpens}</span>
                      </div>
                      ...
                      <div className="hidden md:flex items-center gap-1.5">
                        <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${group.openRate >= 70 ? "bg-emerald-500" : group.openRate >= 40 ? "bg-amber-500" : "bg-red-400"}`}
                  ...
                  <div className="px-3 pb-3 space-y-1">
                    ...
                    <div className="flex items-center gap-2 px-3 py-2 mb-2">
                      ...
                      <div className="flex items-center gap-1 text-xs text-muted-foreground ml-auto">
                        ...
                        Open rate: <span className={`font-medium ${group.openRate >= 70 ? "text-emerald-600" : group.openRate >= 40 ? "text-amber-600" : "text-red-500"}`}>{group.openRate}%</span>
                      </div>
                    </div>
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              <div className={`w-2 h-2 rounded-full shrink-0 ${
                                ...
                                clicks.length > 0 ? "bg-primary" :
                                opens.length > 0 ? "bg-emerald-500" :
                            ...
                            <div className="flex items-center gap-2 shrink-0 ml-2">
                              {bounces.length > 0 ? (
                                ...
                                <>
                                  {opens.length > 0 && <span className="text-[10px] text-emerald-600 font-medium">✓ {opens.length > 1 ? `${opens.length}x` : "Opened"}</span>}
